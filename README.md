...
| Aeon | Ривер Парк Коломенское | `aeon/aeon_riverpark_feed.xml` |
| Sezar Group | СЕЗАР СИТИ | `sezar/sezar_city_feed.xml` |

Все лоты Ривер Парк Коломенское публикуются с категорией `newBuildingFlatSale`,
`FlatRoomsCount=7` (свободная планировка) и полной иерархией
`JKSchema/House/Flat` проекта с CIAN ID `6178`.

В каждом объекте СЕЗАР СИТИ публикуются две PNG-картинки: планировка квартиры
и схема этажа с выделенным расположением лота.
...
API-ключи. Для Ривер Парк Коломенское CIAN ID `6178` задан в генераторе,
поэтому отдельный секрет не требуется.
